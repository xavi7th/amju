<div class="mt-20">
    ...
    <div class="bg-slate-900 border-b border-white/5 py-12">
        <div class="container mx-auto px-6">
            <h1 class="text-4xl md:text-5xl font-black text-white mb-4">Investor <span class="text-amju-primary">Relations</span></h1>
            <nav class="flex text-sm font-bold text-slate-400 uppercase tracking-widest">
                ...
                <a href="{{ route('site.ir') }}" class="{{ $type === 'main' ? 'text-amju-primary' : 'hover:text-amju-primary transition' }}">IR Center</a>
                @if($type !== 'main')
                    <span class="mx-3">/</span>
                    <span class="text-amju-primary uppercase">{{ str_replace(['-', 'ir-'], ' ', $type) }}</span>
                @endif
            </nav>
        </div>
    </div>
                ...
                <main class="lg:w-3/4">
                    @if($type === 'main')
                        <div class="space-y-20">
                            <div class="max-w-3xl">
                                <h2 class="text-4xl font-black text-slate-900 mb-6 font-display">A Wide Range of <span class="text-amju-primary">Financial Growth</span></h2>
                                ...
                                @foreach([
                                    ...
                                    <div class="flex flex-col md:flex-row gap-12 items-center group">
                                        ...
                                        <div class="md:w-1/2 space-y-6">
                                            <h3 class="text-3xl font-black text-slate-900 group-hover:text-amju-primary transition-colors">{{ $service['title'] }}</h3>
                                            <p class="text-slate-500 leading-relaxed font-medium">{{ $service['desc'] }}</p>
                                            <a href="{{ route('contact') }}" class="inline-flex items-center text-amju-primary font-black uppercase tracking-widest text-sm hover:underline">
                    ...
                    @elseif($type === 'financial-information')
                        <div class="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <h2 class="text-4xl font-black text-slate-900 font-display">Financial <span class="text-amju-primary">Reports</span></h2>
                                ...
                                @foreach(['2023', '2022', '2021', '2020'] as $year)
                                    <div class="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex items-center justify-between group hover:bg-white hover:shadow-xl transition-all">
                                        <div class="flex items-center space-x-6">
                                            <div class="w-16 h-16 bg-amju-primary/10 rounded-2xl flex items-center justify-center text-amju-primary">
                                        ...
                                        <a href="#" class="w-12 h-12 bg-slate-200 rounded-full flex items-center justify-center text-slate-500 group-hover:bg-amju-primary group-hover:text-white transition-all">
